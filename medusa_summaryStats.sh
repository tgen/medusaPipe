...
if [ -e $runDir/summaryStatsPass ] ; then
	...
	touch $runDir/medusa_nextJob_finalize.txt
	alreadyDone=1
fi
...
if [ $alreadyDone -eq 0 ] ; then
	...
	qsub -A $debit -l nodes=1:ppn=$nCores -v SUMSTATSPATH=$sumStatsPath,RUNDIR=$runDir,EMAIL=$email,NXT1=$nxtStep1,D=$d $pbsHome/medusa_summaryStats.pbs
	if [ $? -eq 0 ] ; then
