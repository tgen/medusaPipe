...
thisStep="medusa_nextJob_runFastQC.txt"
nxtStep1="medusa_nextJob_postFastQC.txt"
...
do
    ...
    sbatch -n 1 -N 1 --cpus-per-task $nCores --output $runDir/oeFiles/%x-slurm-%j.out --export ALL,FQ=$thisFq,FASTQCPATH=$fastqcPath,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pbsHome/medusa_runFastQC.sh
    if [ $? -eq 0 ] ; then
