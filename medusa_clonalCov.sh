...
do
    ...
    if [[ -e $normalBamFile.clonalCovInQueue || -e $normalBamFile.clonalCovFail || -e $normalBamFile.clonalCovPass ]] ; then
        ...
        if [[ ! -e $normalBamFile || ! -e $normalBaiFile || ! -e $normalBamFile.jointIRPass ]] ; then
            ...
            sbatch -n 1 -N 1 --cpus-per-task $nCores --output $runDir/oeFiles/%x-slurm-%j.out --export ALL,BAMFILE=$normalBamFile,OUTFILE=$normalBamFile.clc,CPATH=$clonalCovPath,SAMPATH=$samtoolsPath,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pbsHome/medusa_clonalCov.sh
            if [ $? -eq 0 ] ; then
    ...
    if [[ -e $tumorBamFile.clonalCovInQueue || -e $tumorBamFile.clonalCovFail || -e $tumorBamFile.clonalCovPass ]] ; then
        ...
        if [[ ! -e $tumorBamFile || ! -e $tumorBaiFile || ! -e $tumorBamFile.jointIRPass ]] ; then
            ...
            sbatch -n 1 -N 1 --cpus-per-task $nCores --output $runDir/oeFiles/%x-slurm-%j.out --export ALL,BAMFILE=$tumorBamFile,OUTFILE=$tumorBamFile.clc,CPATH=$clonalCovPath,SAMPATH=$samtoolsPath,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pbsHome/medusa_clonalCov.sh
            if [ $? -eq 0 ] ; then
