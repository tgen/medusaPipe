...
thisStep="medusa_nextJob_circos.txt"
nxtStep1="medusa_nextJob_postCircos.txt"
...
do
    ...
    sbatch -n 1 -N 1 --cpus-per-task $nCores --output $runDir/oeFiles/%x-slurm-%j.out --export ALL,OUTFILE=$circosSamples,CONF=$conf,OUTDIR=$outDir,SEURATVCF=$seuratVcf,COSMIC=$cosmicVcf,TRNVCF=$trnVcf,CNVTSV=$cnvsTSV,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pbsHome/medusa_circos.sh
    if [ $? -eq 0 ] ; then
