...
thisStep="medusa_nextJob_deSeq2.txt"
nxtStep1="medusa_nextJob_postDeSeq2.txt"
...
do
    ...
    tophat) echo "tophat case"
        ...
        sbatch -n 1 -N 1 --cpus-per-task $nCores --output $runDir/oeFiles/%x-slurm-%j.out --export ALL,DESEQ2PATH=$deseq2Path,RUNDIR=$runDir,DIRNAME=$deSeqDir,GTF=$gtf,NORMLIST="'"$normlList"'",TUMORLIST="'"$tumorList"'",REF=$ref,NXT1=$nxtStep1,D=$d $pbsHome/medusa_deSeq2.pbs
        if [ $? -eq 0 ] ; then
