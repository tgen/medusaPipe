...
do
    ...
    tophat) echo "tophat case"
        ...
        qsub -A $debit -l nodes=1:ppn=$nCores -v DESEQ2PATH=$deseq2Path,RUNDIR=$runDir,DIRNAME=$deSeqDir,GTF=$gtf,NORMLIST="'"$normlList"'",TUMORLIST="'"$tumorList"'",REF=$ref,NXT1=$nxtStep1,D=$d $pbsHome/medusa_deSeq2.pbs
        if [ $? -eq 0 ] ; then
