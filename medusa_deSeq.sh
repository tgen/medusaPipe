...
thisStep="medusa_nextJob_deSeq.txt"
nxtStep1="medusa_nextJob_postDeSeq.txt"
...
do
    ...
    tophat) echo "tophat case"
        ...
        do
            ...
            if [ ! -e $eachNormlHTseqPas ] ; then
                echo "### Can't find normal ht seq pass : $eachNormlHTseqPas"
                ((missingNormlCount++))
            else
        ...
        sbatch -n 1 -N 1 --cpus-per-task $nCores --output $runDir/oeFiles/%x-slurm-%j.out --export ALL,DESEQPATH=$deseqPath,RUNDIR=$runDir,DIRNAME=$deSeqDir,GTF=$gtf,NORMLIST="'"$normlList"'",TUMORLIST="'"$tumorList"'",REF=$ref,NXT1=$nxtStep1,D=$d $pbsHome/medusa_deSeq.pbs
        if [ $? -eq 0 ] ; then
    ...
    star) echo "star case"
        ...
        do
            ...
            if [ ! -e $eachNormlHTseqPas ] ; then
                echo "### Can't find star normal ht seq pass: $eachNormlHTseqPas"
                ((missingNormlCount++))
            else
                normlList="$eachNormlHTseqOut;$normlList"
                normlList2="$eachNorml-$normlList2"
