...
do
    ...
    tophat) echo "tophat case"
        ...
        sbatch -n 1 -N 1 --cpus-per-task $nCores --output $runDir/oeFiles/%x-slurm-%j.out --export ALL,DESEQPATH=$deseqPath,RUNDIR=$runDir,DIRNAME=$deSeqDir,GTF=$gtf,NORMLIST="$normlList",TUMORLIST="$tumorList",REF=$ref,NXT1=$nxtStep1,D=$d $pbsHome/medusa_deSeq.sh
        if [ $? -eq 0 ] ; then
