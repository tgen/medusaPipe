...
thisStep="medusa_nextJob_dnaAlignParts.txt"
nxtStep1="medusa_nextJob_indelRealignParts.txt"
nxtStep2="medusa_nextJob_recalibrateParts.txt"
                    ...
                    do
                        ...
                        do
                            ...
                            bwamem) echo "### Submitting to qbwaMem to create $bamMiniName"
                                ...
                                sbatch -n 1 -N 1 --cpus-per-task $nCores --output $runDir/oeFiles/%x-slurm-%j.out --export ALL,D=$d,RGTAG=$rgTag,FASTQ1=$thisMiniFq,FASTQ2=$thisMiniR2,REF=$ref,BWAPATH=$bwaPath,SAMTOOLSPATH=$samtoolsPath,FAI=$faiFile,BAMPRE=$bamMiniPre,RUNDIR=$runDir,NXT1=$nxtStep1,NXT2=$nxtStep2,D=$d $pbsHome/medusa_bwaMem.sh
                                if [ $? -eq 0 ] ; then
