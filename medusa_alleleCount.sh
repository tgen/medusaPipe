...
thisStep="medusa_nextJob_alleleCount.txt"
nxtStep1="medusa_nextJob_postAlleleCount.txt"
...
do
    ...
    echo "tumorBam=$runDir/$tumorKit/$tumor/$tumor.proj.md.jr.bam"
    echo "jrIRPass=$runDir/$tumorKit/$tumor/$tumor.proj.md.jr.bam.jointIRPass"
    jrIRPass=$runDir/$tumorKit/$tumor/$tumor.proj.md.jr.bam.jointIRPass
    if [[ ! -e $tumorBam || ! -e $jrIRPass ]] ; then
        echo "### Tumor bam or its joint IR Pass does not exist yet"
        ((qsubFails++))
        continue
    fi
    ...
    star) echo "star case"
        ...
        echo "rnaInBam=$runDir/$rnaInKit/$rnaIn/$rnaIn.starDir/$rnaIn.proj.Aligned.out.sorted.bam"
        echo "starPass=$runDir/$rnaInKit/$rnaIn/$rnaIn.starDir.starPass"
    ...
    *) echo "I should not be here"
    ...
    sbatch -n 1 -N 1 --cpus-per-task $nCores --output $runDir/oeFiles/%x-slurm-%j.out --export ALL,OUT=$outFile,ALCOUNTPATH=$alPath,REF=$ref,TRACK=$trackName/$usableName,VCF=$seuratVcf,RNABAM=$rnaInBam,DNABAM=$tumorBam,RUNDIR=$runDir,D=$d $pbsHome/medusa_alleleCount.sh
    if [ $? -eq 0 ] ; then
