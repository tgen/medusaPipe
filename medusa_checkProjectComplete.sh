...
thisStep="medusa_nextJob_checkProjectComplete.txt"
nxtStep1="medusa_nextJob_summaryStats.txt"
...
do
    ...
    if [ "$assayID" != "RNA" ] ; then
        ...
        tophat) #echo "### Tophat case"
            ...
            sailFishPass=$runDir/$kitName/$samName/$samName.topHatDir.sailFishPass
        ...
        star) #echo "### Star case"
            ...
            sailFishPass=$runDir/$kitName/$samName/$samName.sailFishDir.sailFishPass
        ...
        *) echo "### I should not be here."
        ...
        if [ "$sailFish" == "yes" ] ; then
            ((totalChecked++))
            if [ ! -e $sailFishPass ] ; then
                echo "### Missing sailFishPass file for $rnaAligner"
                echo "### Missing file: $sailFishPass"
                ((missingSailFish++))
            fi
        fi
...
###this loop is for finding mutect vcfs
for dnaPairLine in `cat $configFile | grep '^DNAPAIR='`
do
    if [ "$mutect" == "yes" ] ; then
...
## This loop is for finding single bam called vcf files from GATK
for sampleLine in `cat $configFile | grep ^SAMPLE=`
do
    kitName=`echo $sampleLine | cut -d= -f2 | cut -d, -f1`
    samName=`echo $sampleLine | cut -d= -f2 | cut -d, -f2`
    ...
    if [ "$assayID" != "RNA" ] ; then
        ...
        if [ $jrRequested -gt 0 ] ; then
            ...
            #echo "### File check will run on single bam because Joint IR is NOT requested for $samName"
            ((totalChecked++))
        ...
        #code for calling picard metrics on RNA bams here
    fi
done

totalMissing=$(( missingCnaExome + missingCnaGenUnfi +missingCnaGenFilt + missingHtSeq + missingRNAalign + missingRNAmetric +missingGermLineSnpEff + missingSeuratSnpEff + missingMultiSampleHCSnpEff + missingSamStat + missingPicMulti + missingPicHS + missingPicHS2 + missingTrn + missingAlleleCount + missingvcfMergerAC + missingDeSeq + missingCuffDiff + missingCuffLink + missingCuffQuant + missingSnpSniff + missingSailFish + missingIglBedCov + missingFusionDetect + missingMutect + missingStrelka + missingVcfMerger ))
