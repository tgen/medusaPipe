...
do
    ...
    sbatch -n 1 -N 1 --cpus-per-task $nCores --output $runDir/oeFiles/%x-slurm-%j.out --export ALL,SNPEFFPATH=$snpeffPath,VCF=$trackName.seurat.vcf,DBSNP=$dbsnp,DBVERSION=$snpeffdb,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pbsHome/medusa_snpEff.sh
    if [ $? -eq 0 ] ; then
...
do
    ...
    sbatch -n 1 -N 1 --cpus-per-task $nCores --output $runDir/oeFiles/%x-slurm-%j.out --export ALL,SNPEFFPATH=$snpeffPath,VCF=$trackName.REVseurat.vcf,DBSNP=$dbsnp,DBVERSION=$snpeffdb,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pbsHome/medusa_snpEff.sh
    if [ $? -eq 0 ] ; then
...
do
    ...
    if [[ -e $vcfPre.strelka.passed.somatic.snvs.vcf.snpEffInQueue || -e $vcfPre.strelka.passed.somatic.snvs.vcf.snpEffPass || -e $vcfPre.strelka.passed.somatic.snvs.vcf.snpEffFail ]] ; then
        ...
        sbatch -n 1 -N 1 --cpus-per-task $nCores --output $runDir/oeFiles/%x-slurm-%j.out --export ALL,SNPEFFPATH=$snpeffPath,VCF=$vcfPre.strelka.passed.somatic.snvs.vcf,DBSNP=$dbsnp,DBVERSION=$snpeffdb,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pbsHome/medusa_snpEff.sh
        if [ $? -eq 0 ] ; then
    ...
    if [[ -e $vcfPre.strelka.all.somatic.snvs.vcf.snpEffInQueue || -e $vcfPre.strelka.all.somatic.snvs.vcf.snpEffPass || -e $vcfPre.strelka.all.somatic.snvs.vcf.snpEffFail ]] ; then
        ...
        sbatch -n 1 -N 1 --cpus-per-task $nCores --output $runDir/oeFiles/%x-slurm-%j.out --export ALL,SNPEFFPATH=$snpeffPath,VCF=$vcfPre.strelka.all.somatic.snvs.vcf,DBSNP=$dbsnp,DBVERSION=$snpeffdb,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pbsHome/medusa_snpEff.sh
        if [ $? -eq 0 ] ; then
    ...
    if [[ -e $vcfPre.strelka.passed.somatic.indels.vcf.snpEffInQueue || -e $vcfPre.strelka.passed.somatic.indels.vcf.snpEffPass || -e $vcfPre.strelka.passed.somatic.indels.vcf.snpEffFail ]] ; then
        ...
        sbatch -n 1 -N 1 --cpus-per-task $nCores --output $runDir/oeFiles/%x-slurm-%j.out --export ALL,SNPEFFPATH=$snpeffPath,VCF=$vcfPre.strelka.passed.somatic.indels.vcf,DBSNP=$dbsnp,DBVERSION=$snpeffdb,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pbsHome/medusa_snpEff.sh
        if [ $? -eq 0 ] ; then
    ...
    if [[ -e $vcfPre.strelka.all.somatic.indels.vcf.snpEffInQueue || -e $vcfPre.strelka.all.somatic.indels.vcf.snpEffPass || -e $vcfPre.strelka.all.somatic.indels.vcf.snpEffFail ]] ; then
        ...
        sbatch -n 1 -N 1 --cpus-per-task $nCores --output $runDir/oeFiles/%x-slurm-%j.out --export ALL,SNPEFFPATH=$snpeffPath,VCF=$vcfPre.strelka.all.somatic.indels.vcf,DBSNP=$dbsnp,DBVERSION=$snpeffdb,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pbsHome/medusa_snpEff.sh
        if [ $? -eq 0 ] ; then
...
do
    ...
    sbatch -n 1 -N 1 --cpus-per-task $nCores --output $runDir/oeFiles/%x-slurm-%j.out --export ALL,SNPEFFPATH=$snpeffPath,VCF=$vcf,DBSNP=$dbsnp,DBVERSION=$snpeffdb,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pbsHome/medusa_snpEff.sh
    if [ $? -eq 0 ] ; then
...
do
    ...
    sbatch -n 1 -N 1 --cpus-per-task $nCores --output $runDir/oeFiles/%x-slurm-%j.out --export ALL,SNPEFFPATH=$snpeffPath,VCF=$trackName.HC_All.vcf,DBSNP=$dbsnp,DBVERSION=$snpeffdb,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pbsHome/medusa_snpEff.sh
    if [ $? -eq 0 ] ; then
        ...
        if [ $jrRequested -gt 0 ] ; then
            ...
            if [[ ! -e $hcPas || ! -e $hcVcf ]] ; then
                ...
                if [[ -e $hcVcf.snpEffPass || -e $hcVcf.snpEffInQueue || -e $hcVcf.snpEffFail ]] ; then
                    ...
                    sbatch -n 1 -N 1 --cpus-per-task $nCores --output $runDir/oeFiles/%x-slurm-%j.out --export ALL,SNPEFFPATH=$snpeffPath,VCF=$hcVcf,DBSNP=$dbsnp,DBVERSION=$snpeffdb,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pbsHome/medusa_snpEff.sh
                    if [ $? -eq 0 ] ; then
