...
thisStep="medusa_nextJob_snpSniff.txt"
...
do
    ...
    if [[ "$assayID" == "Exome" || "$assayID" == "Genome" || "$assayID" == "exome" ]] ; then
        ...
        if [ ! -e $pasFile ] ; then
            ...
            if [[ -e $bamFile.snpSniffInQueue || -e $bamFile.snpSniffFail || -e $bamFile.snpSniffPass ]] ; then
                ...
                sbatch -n 1 -N 1 --cpus-per-task $nCores --output $runDir/oeFiles/%x-slurm-%j.out --export ALL,SAMTOOLSPATH=$samtoolsPath,OUTVCF=$finalOut,REF=$ref,SNPSNIFFERPATH=$snpSnifferPath,BAM=$bamFile,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pbsHome/medusa_snpSniffer.pbs
                if [ $? -eq 0 ] ; then
        ...
        *) echo "I should not be here"
        ...
        if [ ! -e $pasFile ] ; then
            ...
            if [[ -e $bamFile.snpSniffInQueue || -e $bamFile.snpSniffFail || -e $bamFile.snpSniffPass ]] ; then
                ...
                echo "### Submitting for snpSniffer: $bamFile"
                sbatch -n 1 -N 1 --cpus-per-task $nCores --output $runDir/oeFiles/%x-slurm-%j.out --export ALL,SAMTOOLSPATH=$samtoolsPath,REF=$ref,OUTVCF=$finalOut,SNPSNIFFERPATH=$snpSnifferPath,BAM=$bamFile,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pbsHome/medusa_snpSniffer.pbs
                if [ $? -eq 0 ] ; then
                    touch $bamFile.snpSniffInQueue
                else
                    ((qsubFails++))
