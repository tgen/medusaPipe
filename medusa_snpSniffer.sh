	...
	if [[ "$assayID" == "Exome" || "$assayID" == "Genome" || "$assayID" == "exome" ]] ; then
		...
		if [ ! -e $pasFile ] ; then
			...
			if [[ -e $bamFile.snpSniffInQueue || -e $bamFile.snpSniffFail || -e $bamFile.snpSniffPass ]] ; then
				...
				qsub -A $debit -l nodes=1:ppn=$nCores -v SAMTOOLSPATH=$samtoolsPath,OUTVCF=$finalOut,REF=$ref,SNPSNIFFERPATH=$snpSnifferPath,BAM=$bamFile,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pbsHome/medusa_snpSniffer.pbs
				if [ $? -eq 0 ] ; then
		...
		*) echo "I should not be here"
		...
		if [ ! -e $pasFile ] ; then 
			...
			if [[ -e $bamFile.snpSniffInQueue || -e $bamFile.snpSniffFail || -e $bamFile.snpSniffPass ]] ; then
				...
				echo "### Submitting for snpSniffer: $bamFile"
				qsub -A $debit -l nodes=1:ppn=$nCores -v SAMTOOLSPATH=$samtoolsPath,REF=$ref,OUTVCF=$finalOut,SNPSNIFFERPATH=$snpSnifferPath,BAM=$bamFile,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pbsHome/medusa_snpSniffer.pbs
				if [ $? -eq 0 ] ; then
					touch $bamFile.snpSniffInQueue
				else
					((qsubFails++))
