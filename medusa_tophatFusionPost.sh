...
do
    ...
    qsub -A $debit -l nodes=1:ppn=$nCores -v THFUSION2VCFPATH=$thFusion2vcfPath,TOPHAT2PATH=$tophat2Path,RUNDIR=$runDir,DIR=$topHatFDir,INDEXBASE=$indexbase,NXT1=$nxtStep1,REF=$thFusionRef,D=$d $pbsHome/medusa_tophatFusionPost.pbs
    if [ $? -eq 0 ] ; then
