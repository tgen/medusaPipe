            ...
            if [ $arrayCount -gt 0 ] ; then
                ...
                if [ $skipGroup -ne 1 ] ; then
                    ...
                    do
                        ...
                        qsub -A $debit -l nodes=1:ppn=$nCores -v GATKPATH=$gatkPath,INTS=$irIntFile,IRBAMFILE=$irBamFile,D=$d,INDELS=$indels,REF=$ref,BAMFILE=$bamName,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pbsHome/medusa_indelRealign.pbs
                        if [ $? -eq 0 ] ; then
