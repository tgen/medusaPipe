...
do
	...
	*) echo "### I should not be here"
	...
	if [ $rnaStrand == "FIRST" ] ; then
		...
		qsub -A $debit -l nodes=1:ppn=$nCores -v REF=$ref,REFFLAT=$refFlat,RIBINTS=$ribInts,PICARDPATH=$picardPath,BAMFILE=$rnaBam,RUNDIR=$runDir,D=$d $pbsHome/medusa_FSpicardRNAMetrics.pbs
		if [ $? -eq 0 ] ; then
		...
		sleep 2
                ...
                echo "##running stranded picard metrics case"
                qsub -A $debit -l nodes=1:ppn=$nCores -v REF=$ref,REFFLAT=$refFlat,RIBINTS=$ribInts,PICARDPATH=$picardPath,BAMFILE=$rnaBam,RUNDIR=$runDir,D=$d $pbsHome/medusa_SSpicardRNAMetrics.pbs
                if [ $? -eq 0 ] ; then
                        touch $rnaBam.picRNAMetricsInQueue
                else
                        ((qsubFails++))
		...
		qsub -A $debit -l nodes=1:ppn=$nCores -v REF=$ref,REFFLAT=$refFlat,RIBINTS=$ribInts,PICARDPATH=$picardPath,BAMFILE=$rnaBam,RUNDIR=$runDir,D=$d $pbsHome/medusa_picardRNAMetrics.pbs
		if [ $? -eq 0 ] ; then
			touch $rnaBam.picRNAMetricsInQueue
		else
			((qsubFails++))
