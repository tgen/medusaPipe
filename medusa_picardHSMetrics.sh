...
do
    ...
    if [ "$assayID" == "Exome" ] ; then
        ...
        #        qsub -A $debit -l nodes=1:ppn=$nCores -v BAITS=$baits,TARGETS=$targets,PICARDPATH=$picardPath,RUNDIR=$runDir,REF=$ref,BAMFILE=$inBam,DIR=$pcDir,NXT1=$nxtStep1,D=$d $pbsHome/medusa_picardHSMetrics.pbs
        #        if [ $? -eq 0 ] ; then
...
#        fi
        if [[ ! -e $inBam.mdPass || ! -e $mdBam ]] ; then
            ...
            if [[ -e $mdBam.picHSMetricsPass || -e $mdBam.picHSMetricsInQueue || -e $mdBam.picHSMetricsFail ]] ; then
                ...
                qsub -A $debit -l nodes=1:ppn=$nCores -v BAITS=$baits,TARGETS=$targets,PICARDPATH=$picardPath,RUNDIR=$runDir,REF=$ref,BAMFILE=$mdBam,DIR=$pcDir,NXT1=$nxtStep1,D=$d $pbsHome/medusa_picardHSMetrics.pbs
                if [ $? -eq 0 ] ; then
        ...
        if [ $jrRequested -gt 0 ] ; then
            ...
            if [[ ! -e $jrBam.jointIRPass || ! -e $jrBam ]] ; then
                ...
                if [[ -e $jrBam.picHSMetricsPass || -e $jrBam.picHSMetricsInQueue || -e $jrBam.picHSMetricsFail ]] ; then
                    ...
                    qsub -A $debit -l nodes=1:ppn=$nCores -v BAITS=$baits,TARGETS=$targets,PICARDPATH=$picardPath,RUNDIR=$runDir,REF=$ref,BAMFILE=$jrBam,DIR=$pcDir,NXT1=$nxtStep1,D=$d $pbsHome/medusa_picardHSMetrics.pbs
                    if [ $? -eq 0 ] ; then
