...
thisStep="medusa_nextJob_cna.txt"
nxtStep1="medusa_nextJob_postCna.txt"
...
do
    ...
    GenomeGenome) echo "### Both normal and tumor are Genome"
            ...
            if [ "$cnaGenomeFilter" = "yes" ] ; then
                ...
                if [[ -e $trackName.cnaGenFiltInQueue || -e $trackName.cnaGenFiltPass || -e $trackName.cnaGenFiltFail ]] ; then
                    ...
                    sbatch -n 1 -N 1 --cpus-per-task $nCores --output $runDir/oeFiles/%x-slurm-%j.out --export ALL,TRACKNAME=$trackName,MYPATH=$cnaDir/$outName1,CNAPATH=$cnaPath,MERGEDVCF=$vcfFile,NORMALSAMPLE=$pair1SamName,TUMORSAMPLE=$pair2SamName,NORMALDAT=$normalDatFile,TUMORDAT=$tumorDatFile,OFILE=$outName1,ASSAY=$assay,GTF=$gtf,NXT1=$nxtStep1,REF=$ref,RUNDIR=$runDir,D=$d $pbsHome/medusa_cnaGenFilt.sh
                    if [ $? -eq 0 ] ; then
            ...
            if [ "$cnaGenomeUnFilter" = "yes" ] ; then
                ...
                if [[ -e $trackName.cnaGenUnfiInQueue || -e $trackName.cnaGenUnfiPass || -e $trackName.cnaGenUnfiFail ]] ; then
                    ...
                    echo "### Submitting to queue with $trackName"
                    sbatch -n 1 -N 1 --cpus-per-task $nCores --output $runDir/oeFiles/%x-slurm-%j.out --export ALL,TRACKNAME=$trackName,MYPATH=$cnaDir/$outName2,CNAPATH=$cnaPath,MERGEDVCF=$vcfFile,NORMALSAMPLE=$pair1SamName,TUMORSAMPLE=$pair2SamName,NORMALDAT=$normalDatFile,TUMORDAT=$tumorDatFile,OFILE=$outName2,ASSAY=$assay,GTF=$gtf,NXT1=$nxtStep1,REF=$ref,RUNDIR=$runDir,D=$d $pbsHome/medusa_cnaGenUnfi.sh
                    if [ $? -eq 0 ] ; then
    ...
    ExomeExome) echo "### Both normal and tumor are Exome"
            ...
            if [[ -e $trackName.cnaExomeInQueue || -e $trackName.cnaExomePass || -e $trackName.cnaExomeFail ]] ; then
                ...
                echo "### Submitting to queue with $trackName"
                sbatch -n 1 -N 1 --cpus-per-task $nCores --output $runDir/oeFiles/%x-slurm-%j.out --export ALL,NHETDEPTH=$nHetDepth,THETDEPTH=$tHetDepth,TRACKNAME=$trackName,MYPATH=$cnaDir/$outName3,CNAPATH=$cnaPath,MERGEDVCF=$vcfFile,NORMALSAMPLE=$pair1SamName,TUMORSAMPLE=$pair2SamName,CNAEXOMETARGET=$bedFile,NORMALDAT=$normalDatFile,TUMORDAT=$tumorDatFile,OFILE=$outName3,ASSAY=$assay,GTF=$gtf,NXT1=$nxtStep1,REF=$ref,RUNDIR=$runDir,D=$d $pbsHome/medusa_cnaExo.sh
                if [ $? -eq 0 ] ; then
