#!/usr/bin/env bash
#PBS -S /bin/bash
#SBATCH --job-name="medusa_vcfMergerAC"
...
echo "${POST_MERGE_VENN} --vcf ${MERGERDIR}/${BASENAME}.merge.sort.clean.f2t.ann.rna.dbnsfp.se74lofcan.vcf --dirscript ${VCFMERGER_DIR} --outprefix  ${MERGERDIR}/${BASENAME}_finalVenn  --maintitle ${BASENAME} --"
${POST_MERGE_VENN} --vcf ${MERGERDIR}/${BASENAME}.merge.sort.clean.f2t.ann.rna.dbnsfp.se74lofcan.vcf --dirscript ${VCFMERGER_DIR} --outprefix  ${MERGERDIR}/${BASENAME}_finalVenn  --maintitle ${BASENAME} --
if [ $? -ne 0 ] ; then
...
echo "rm -rf ${MERGERDIR}/${BASENAME}.vcfMergerACInQueue"
...
echo "RUNTIME:SNPEFF:$hours:$mins" > ${MERGERDIR}/${BASENAME}.vcfMerger.totalTime
time=`date +%d-%m-%Y-%H-%M` 
