...
thisStep="medusa_nextJob_indelRealignParts.txt"
nxtStep1="medusa_nextJob_recalibrateParts.txt"
                ...
                if [ $skipGroup -ne 1 ] ; then
                    ...
                    do
                        ...
                        do
                            ...
                            sbatch -n 1 -N 1 --cpus-per-task $nCores --output $runDir/oeFiles/%x-slurm-%j.out --export ALL,GATKPATH=$gatkPath,INTS=$irIntMiniFile,IRBAMFILE=$irBamMiniFile,D=$d,INDELS=$indels,REF=$ref,BAMFILE=$bamMiniName,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pbsHome/medusa_indelRealign.pbs
                            if [ $? -eq 0 ] ; then
