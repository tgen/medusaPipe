                ...
                if [ $skipGroup -ne 1 ] ; then
                    ...
                    do
                        ...
                        do
                            ...
                            sbatch -n 1 -N 1 --cpus-per-task $nCores --output $runDir/oeFiles/%x-slurm-%j.out --export ALL,GATKPATH=$gatkPath,INTS=$irIntMiniFile,IRBAMFILE=$irBamMiniFile,D=$d,INDELS=$indels,REF=$ref,BAMFILE=$bamMiniName,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pbsHome/medusa_indelRealign.sh
                            if [ $? -eq 0 ] ; then
