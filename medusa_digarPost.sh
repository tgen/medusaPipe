...
thisStep="medusa_nextJob_digarPost.txt"
nxtStep1="medusa_nextJob_postdigarPost.txt"
...
do
       if [[ "$digar" != "yes" ]] ; then
        ...
        star) echo "star case"
            ...
            sbatch -n 1 -N 1 --cpus-per-task $nCores --output $runDir/oeFiles/%x-slurm-%j.out --export ALL,DIGARPATH=$digarPath,ANN=$digarAnn,SAMTOOLSPATH=$samtoolsPath,BWAPATH=$bwaPath,GENEFILE=$listOfGenes,DIGARDIR=$digarDir,REF=$ref,BAM=$starBam,GTF=$gtf,RUNDIR=$runDir,D=$d $pbsHome/medusa_digarPost.sh
            if [ $? -eq 0 ] ; then
