...
thisStep="medusa_nextJob_mergeBams.txt"
nxtStep1="medusa_nextJob_markDups.txt"
            ...
            if [ $arrayCount -gt 0 ] ; then
                ...
                if [ $missingBamSample -eq 0 ] ; then
                    ...
                    if [[ -e $mergedBamName.mergeBamPass || -e $mergedBamName.mergeBamInQueue || -e $mergedBamName.mergeBamFail ]] ; then
                        ...
                        sbatch -n 1 -N 1 --cpus-per-task $nCores --output $runDir/oeFiles/%x-slurm-%j.out --export ALL,PICARDPATH=$picardPath,SAMTOOLSPATH=$samtoolsPath,CNT=$bamCount,RUNDIR=$runDir,NXT1=$nxtStep1,BAMLIST="$bamList",MERGEDBAM=$mergedBamName,D=$d $pbsHome/medusa_mergeBams.pbs
                        if [ $? -eq 0 ] ; then
