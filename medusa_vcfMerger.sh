...
if [[ "$recipe" == "choc01"  ]] ; then
    COSMIC="/home/tgenref/binaries/vcfMerger/vcfMerger/CosmicCodingMuts_v66_20130725_withCHR.sorted.vcf"
else
    COSMIC="/home/tgenref/binaries/vcfMerger/vcfMerger/CosmicCodingMuts_v66_20130725_sorted.vcf"
fi
if [[ "$recipe" == "choc01"  ]] ; then
    KG="/home/tgenref/binaries/vcfMerger/vcfMerger/1000G_phase1.snps.high_confidence.withCHR.b37.vcf"
else
    KG="/home/tgenref/binaries/vcfMerger/vcfMerger/1000G_phase1.snps.high_confidence.b37.vcf"
fi

if [[ "$recipe" == "choc01"  ]] ; then
    NHLBI="/home/tgenref/binaries/vcfMerger/vcfMerger/ESP6500SI-V2_snps_indels_withCHR.vcf"

elif [[ "$recipe" == "TPRC" ]] ; then

    NHLBI="/home/tgenref/binaries/vcfMerger/vcfMerger/ESP6500SI-V2_snps_indels_TPRC.vcf"
else 
...
do
    ...
    if [[ "$kitName" == "TSE61" ]] ; then
        ...
        bedFile="/home/tgenref/annotations/dog/canfam3/vcfMergerBed/agilent_canine_exonV2_targets.padded100.bed"  #TODO missing
    ...
    if [ ! -e $vcf.snpEffPass  ] ; then
        ...
        echo "sbatch -n 1 -N 1 --cpus-per-task $nCores --output $runDir/oeFiles/%x-slurm-%j.out --export ALL,SNPEFFPATH=$snpeffPath,CONTROL=$control,TUMOR=$tumor,SNPSIFT=$snpSift,DBNSFP=$DBNSFP,DBNSP=$DBNSP,SAMTOOLS=$samTools,VARSCAN=$varScan,REF=$ref,DICT=$refDict,COSMIC=$COSMIC,KG=$KG,NHLBI=$NHLBI,SNPS=$snps,INDELS=$indels,GATK=$gatkPath,VCFMERGER=$VCFMERGER,VCFMERGER_DIR=$VCFMERGER_DIR,VCFSORTER=$VCFSORTER,RNA_VCF_HEADER=$RNA_VCF_HEADER,POST_MERGE_VENN=$POST_MERGE_VENN,DBSNP=$dbsnp,DBVERSION=$snpeffdb,SEURAT_VCF=$seuratVcf,MUTECT_VCF=$mutectVcf,STRELKA_SNV_VCF=$strelkaSnvVcf,STRELKA_INDEL_VCF=$strelkaIndelVcf,MERGERDIR=$mergerDir,RNABAM=$rnaBam,ASSAYID=$assayID,BEDFILE=$bedFile,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pbsHome/medusa_vcfMerger.sh"
        sbatch -n 1 -N 1 --cpus-per-task $nCores --output $runDir/oeFiles/%x-slurm-%j.out --export ALL,SNPEFFPATH=$snpeffPath,CONTROL=$control,TUMOR=$tumor,SNPSIFT=$snpSift,DBNSFP=$DBNSFP,DBNSP=$DBNSP,SAMTOOLS=$samTools,VARSCAN=$varScan,REF=$ref,DICT=$refDict,COSMIC=$COSMIC,KG=$KG,NHLBI=$NHLBI,SNPS=$snps,INDELS=$indels,GATK=$gatkPath,VCFMERGER=$VCFMERGER,VCFMERGER_DIR=$VCFMERGER_DIR,VCFSORTER=$VCFSORTER,RNA_VCF_HEADER=$RNA_VCF_HEADER,POST_MERGE_VENN=$POST_MERGE_VENN,DBSNP=$dbsnp,DBVERSION=$snpeffdb,SEURAT_VCF=$seuratVcf,MUTECT_VCF=$mutectVcf,STRELKA_SNV_VCF=$strelkaSnvVcf,STRELKA_INDEL_VCF=$strelkaIndelVcf,MERGERDIR=$mergerDir,RNABAM=$rnaBam,ASSAYID=$assayID,BEDFILE=$bedFile,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pbsHome/medusa_vcfMerger.sh
